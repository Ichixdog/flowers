      ...
      this.productService
        ...
        .subscribe((data: ProductType) => {
          ...
          this.cartService.getCart().subscribe((cartData: CartType | DefaultResponseType) => {
            ...
            const cartDataResonse = cartData as CartType

            if(cartDataResonse) {
              const productInCart = cartDataResonse.items.find((item: any) => item.product.id === data.id)
              if(productInCart){
          ...
          if(!this.authService.getIsLoggedIn()){
            this.favoriteService.getFavorites().subscribe((data: FavoriteType[] | DefaultResponseType) => {
              ...
              const currentProductExist= products.find(product => product.id === this.product.id)
